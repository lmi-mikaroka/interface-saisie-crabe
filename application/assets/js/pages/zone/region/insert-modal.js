  ...
  formulaire.on('submit', e => {
    ...
    if (champObligatoireOk()) {
      const attente = $.alert({
        ...
        content: function () {
          return $.ajax({
            url: `${BASE_URL}/edition-de-zone/region/insertion`,
            data: { nom: form.nom.val(), zoneCorecrabe: form.zoneCorecrabe.val() },
            type: 'post',
            dataType: 'json'
          }).done((reponse) => {
            if (!reponse.success) {
              ...
              attente.setContent('L\'insertion de la Région a été effectuée')
