$(() => {
  var datatable = $('#datatable').DataTable({
    ...
    order: [[0, 'asc']],
    ajax: {
      url: BASE_URL + '/edition-de-zone/region/datatable',
      method: 'post',
    },
  });
  ...
  $(document).on('click', '.delete-button', e => {
    ...
    jConfirmRed('Supprimer la sélection', 'La suppression pourra entraîner la perte d\'autres données relatives à celle-ci.\nVeuillez confirmer la suppression', () => {
      $.ajax({
        url: BASE_URL + '/edition-de-zone/region/suppression/' + information,
        method: 'get',
        dataType: 'json',
        success: () => {
  ...
  $(document).on('click', '.update-button', e => {
    ...
    $(target).modal('show');
    $.ajax({
      url: `${BASE_URL}/edition-de-zone/region/selection/${id}`,
      method: 'get',
      dataType: 'json',
      success: response => {
        supprimerValidation()
        form.id.val(response['id'])
        form.zoneCorecrabe.val(response['zone_corecrabe'])
