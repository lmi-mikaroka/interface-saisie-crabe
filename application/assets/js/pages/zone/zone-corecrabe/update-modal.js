  ...
  formulaire.on('submit', e => {
    ...
    if (champObligatoireOk()) {
      const attente = $.alert({
        ...
        content: function () {
          return $.ajax({
            url: `${BASE_URL}/edition-de-zone/zone-corecrabe/mise-a-jour`,
            data: { id: form.id.val(), zoneCorecrabeNom: form.nom.val() },
            type: 'post',
            dataType: 'json'
          }).done((reponse) => {
            if (!reponse.success) {
              ...
              attente.setContent('La modification de la Zone CORECRABE a été effectuée')
