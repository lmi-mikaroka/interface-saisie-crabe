  ...
  formulaire.on('submit', e => {
    ...
    if (champObligatoireOk()) {
      const attente = $.alert({
        ...
        content: function () {
          return $.ajax({
            url: `${BASE_URL}/edition-de-zone/district/mise-a-jour`,
            data: { id: form.id.val(), districtNom: form.nom.val(), region: form.region.val() },
            type: 'post',
            dataType: 'json'
          }).done((reponse) => {
            if (!reponse.success) {
              ...
              attente.setContent('La modification du District a été effectué')
