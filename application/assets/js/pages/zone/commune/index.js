$(() => {
  var datatable = $('#datatable').DataTable({
    ...
    order: [[0, 'asc']],
    ajax: {
      url: BASE_URL + '/edition-de-zone/commune/datatable',
      method: 'post',
    },
  });
  ...
  $(document).on('click', '.delete-button', e => {
    ...
    jConfirmRed('Supprimer la sélection', 'La suppression pourra entraîner la perte d\'autres données relatives à celle-ci.\nVeuillez confirmer la suppression', () => {
      $.ajax({
        url: BASE_URL + '/edition-de-zone/commune/suppression/' + information,
        method: 'get',
        dataType: 'json',
        success: () => {
          $(document).trigger('reload-datatable');
          $.alert('La commune a été supprimée avec succès');
        }
      });
    });
  });
  ...
  $(document).on('click', '.update-button', e => {
    ...
    $(target).modal('show');
    $.ajax({
      url: `${BASE_URL}/edition-de-zone/commune/selection/${id}`,
      method: 'get',
      dataType: 'json',
      success: response => {
        supprimerValidation()
        form.id.val(response['id'])
        form.district.val(response['district'])
