$(() => {
  ...
  const nouveauFicheLien = `${BASE_URL}/saisie-de-fiche-acheteur/saisie-enquete/`
  ...
  form.zoneCorecrabe.on('change', e => {
    ...
    $.ajax({
      ...
      url: BASE_URL + '/edition-de-zone/village/selection-par-zone-corecrabe/' + zone + '/?type=Acheteur',
  ...
  form.village.on('change', e => {
    ...
    actualiserListeEnqueteur(form.village.val(), 3).then(enqueteurs => {
  ...
  form.corps.on('submit', e => {
    ...
    const formularieValide = champObligatoireOk() && champNombreRespecteMinMax();
