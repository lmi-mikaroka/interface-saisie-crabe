  ...
  */
  ...
  form.date.on('change', e => {
    ...
    if (e.currentTarget.value === '') {
      ...
      // Changer l'affichage des dates dans de dernier capture de crabe
  ...
  $('#js-bouton-enregistrer').on('click', e => {
    ...
    const boutonValidations = $('#js-bouton-enreigstrer');
  ...
  function enregistrerFormulaire() {
    return new Promise((resolve, reject) => {
      ...
      if (!boutonValidations.is(':disabled') && minMaxOk && obligatoireOk && autreValidationOk) {
        ...
        $.ajax({
          url: `${BASE_URL}/modification-de-fiche-acheteur/enregistrer`,
          method: 'post',
          data: donnee,
          dataType: 'json',
          success: reponse => {
  ...
  function mettreAJourLesDonneesDePost() {
    ...
    for (let indexe = 0; indexe < 4; indexe++) {
      ...
      donnee.derniereSortieDePeches.push({
  ...
  function champObligatoireComplet() {
    ...
    const requises = [
      ...
      form.venteCrabe.collecte[0].prix,
