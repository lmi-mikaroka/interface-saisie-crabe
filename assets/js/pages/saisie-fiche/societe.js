...
$('input:radio[name=triage]').change(function () {
            if ($("input[name='triage']:checked").val() == 'true') {
                form.trie = true
		toastr.success('Faire attention! Il faut entrer le poids de la cargaison APRES triage');
		$("#message").html("****APRES triage****")
            }
            if ($("input[name='triage']:checked").val() == 'false') {
                form.trie = false
		toastr.info('Faire attention! Il faut entrer le poids de la cargaison AVANT triage');
		$("#message").html("****AVANT triage****")
            }
console.log(form.trie)
        });
    ...
    if(form.datedebarquement != null && form.datedebarquement != ""){
        if(form.zone !=null && form.zone != ""){
            if(form.societe != null && form.societe != ""){
                    if(form.poidstotalcargaison != null && form.poidstotalcargaison >= 18 && form.poidstotalcargaison <= 20000){
                    ...
                    }else return {'result':false, 'message':'Le poids total de la cargaison doit être compris entre 18kg à 20000kg!'}
...
action.zone.change(function(){
  ...
  let optionTags = '';
    ...
    while (action.villages.children().length > 0) {

      action.villages.children().eq(0).remove();
    ...
    $.ajax({
     ...
     url: BASE_URL + '/edition-de-zone/village/selection-par-zone-corecrabe01/' + action.zone.val() ,

      method: 'post',

      data:{zone:action.zone.val()},
    ...
    //    console.log(villages)
        ...
        action.villages.html(optionTags)
      ...
      action.villages.trigger('change')
    })
})
