$(() => {
  ...
  const nouveauFicheLien = `${BASE_URL}/saisie-de-fiche-enqueteur/saisie-enquete/`
  ...
  form.zoneCorecrabe.on('change', e => {
    ...
    $.ajax({
      ...
      url: BASE_URL + '/edition-de-zone/village/selection-par-zone-corecrabe/' + zone + '/?type=Enquêteur',
  ...
  form.village.on('change', e => {
    ...
    actualiserListeEnqueteur(form.village.val(), 1).then(enqueteurs => {
  ...
  form.corps.on('submit', e => {
    ...
    const obligatoireOk = champObligatoireValide()

    const minMaxOk = champNombreRespecteMinMax()

    if (!form.soumission.is(':disabled') && obligatoireOk && minMaxOk) {
  ...
  function champObligatoireValide() {
