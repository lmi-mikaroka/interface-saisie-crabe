$(() => {
  ...
  boutons.enregistrerEtNouveau.on('click', e => {
   ...
   var donnee = mettreAJourLesDonneesDePost();
   console.log(donnee);
    enregistrerFormulaire().then(() => {
      ...
      boutons.enregistrerEtNouveau.removeAttr('disabled')
  ...
  function enregistrerFormulaire() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!boutons.enregistrerEtListe.is(':disabled') && !boutons.enregistrerEtNouveau.is(':disabled') && formulaireValide) {

        $('.js-formulaire-card .js-alerte-information-incorrecte').slideUp(100);

        boutons.enregistrerEtListe.attr('disabled', true);

        boutons.enregistrerEtNouveau.attr('disabled', true);

        $.ajax({

          url: `${BASE_URL}/saisie-de-fiche-recensement/saisie-enquete/insertion`,
