  ...
  formulaire.on('submit', e => {
    ...
    if (champObligatoireOk()) {
      const attente = $.alert({
        ...
        content: function () {
          return $.ajax({
            url: BASE_URL + '/edition-de-zone/commune/mise-a-jour',
            data: { id: form.id.val(), communeNom: form.nom.val(), district: form.district.val() },
            type: 'post',
            dataType: 'json'
          }).done((reponse) => {
            if (!reponse.success) {
              ...
              attente.setContent('La modification de la Commune a été effectué')
